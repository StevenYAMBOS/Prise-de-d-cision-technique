...
- [I - Smash Here](#i---smash-here)
  ...
  - [B - Vision](#b---vision)
    - [1 - Scalabilité](#1---scalabilité)
    - [2 - Sécurité](#2---sécurité)
    - [3 - Évolutivité](#3---évolutivité)
    - [4 - Coûts](#4---coûts)
...
| **Go**          | 2        | Expérience de quelques side-projects, mais sans grande profondeur.                        |
| **Python**      | 3        | A travaillé sur quelques projets, une certaine familiarité.                               |
| **ReactJS**     | 3        | Utilisé régulièrement, le développeur est familier avec son écosystème.                   |
...
| **Fiber**       | 2        | Expérience de quelques side-projects, mais sans grande profondeur.                        |
...
Nous utiliserons **TypeScript** comme langage Front-End. Comme mentionné précédemment, **TypeScript** est une surcouche de **JavaScript**, qui est idéal pour le développement d'applications côté client.

L'utilisation de TypeScript se justifie par sa **sécurité** (grâce au typage statique) et son **efficacité**. Il est **compatible avec tous les frameworks Front-End** et offre un large panel de fonctionnalités.

De plus, le développeur ayant une **bonne maîtrise de JavaScript**, TypeScript permet de continuer à exploiter cet écosystème tout en ajoutant des fonctionnalités supplémentaires.

In fine, **TypeScript** nous offrira une **meilleure gestion du code à long terme**, facilitant ainsi la **maintenance** et l’**évolution de l'application** à mesure qu'elle se développe.
...
Nous utiliserons **SvelteJS** comme framework Front-End. Ce choix est justifié par plusieurs **aspects clés** pour le développement d'un **MVP** :

- **Développement rapide** grâce à sa **simplicité et sa structure intuitive**, ce qui est **crucial en phase initiale**.
- **Facilité d’apprentissage**, réduisant ainsi le temps nécessaire pour obtenir un produit fonctionnel.
- Possibilité de développement de **Progressive Web Apps (PWA)**, ce qui représente un atout stratégique pour offrir une **expérience mobile de qualité** sans devoir développer une application native.
...
Nous avons décidé d'utiliser **Rust** comme langage Back-End en raison de ses **performances exceptionnelles** et de sa **robustesse en gestion mémoire**.

**Rust offre plusieurs avantages :**

- **Sécurité mémoire sans garbage collector** → réduit drastiquement les erreurs de segmentation et améliore la stabilité du code.
- **Performances de bas niveau comparables au C++** → optimise les **temps de réponse du serveur**.
- **Éco-responsabilité** → une **exécution plus efficace** signifie **une consommation énergétique réduite**, un point essentiel pour une plateforme scalable.
- **Excellente gestion du multi-threading** → idéal pour un **service web performant** capable de gérer plusieurs requêtes simultanément.
- **Adopté par des entreprises majeures** (_Discord, AWS, Cloudflare_), témoignant ainsi de sa **fiabilité** et de son **potentiel à long terme**.

#### **Justifications :**

- **Performance optimale (comparable au C++ avec une sécurité accrue)**
- **Gestion mémoire sécurisée sans garbage collector**
- **Concurrence et multi-threading efficace**
- **Langage éco-responsable (moins gourmand en énergie)**

### **D - Framework Back-End**

**Axum**

Pour accompagner **Rust**, nous avons choisi **Axum**, un framework web performant basé sur **Tokio**, optimisé pour les **applications asynchrones et hautement concurrentes**.

**Pourquoi Axum ?**

- **Optimisé pour Rust** → exploite pleinement la **sécurité** et les **performances** du langage.
- **Haute scalabilité** → support natif de la **programmation asynchrone avec Tokio**.
- **Facilité d’intégration avec WebSockets et GraphQL** → idéal pour des **fonctionnalités interactives**.
- **Simplicité et rapidité** → syntaxe **claire et intuitive**, comparable à **FastAPI** en **Python**, mais avec les performances **supérieures de Rust**.

#### **Justifications :**

- **Framework Rust moderne et optimisé pour la performance**
- **Gestion native des WebSockets et des API GraphQL**
- **Asynchrone avec une gestion efficace des requêtes simultanées**
...
Le choix de la base de données s’oriente vers **MongoDB**, principalement pour sa **flexibilité**.

L'application étant encore en construction, **de nombreux changements structurels sont à prévoir**. Dans un **contexte de forte variabilité des données**, **MongoDB** s'avère être **une option idéale**.

**MongoDB est particulièrement adapté pour :**

- **Les données non structurées ou semi-structurées** (_profils de joueurs, roadmaps personnalisées_).
- **Les applications manipulant de gros volumes de données**.
- **Une scalabilité horizontale élevée**, essentielle pour un projet en expansion.
...
|**Framework Front-End**|SvelteJS|
|**Langage Back-End**|Rust|
|**Framework Back-End**|Axum|
